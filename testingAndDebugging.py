...
def test_dif_start_mins(models, train_folders, test_folders, change, mixed_pred_func, first_mins)
    ...
    for min in first_mins:
        print(f'Now running using the first {min} minutes')
...
def main():
    ...
    for minutes in [8, 10, 15, 20, 30, 50, 100, 200, 300, 400, 500, 550, 590, 595]:
        print(f'Beginning with minutes = {minutes}')
        for pair in splits:
            train_folders = pair[0]
            test_folders = pair[1]
            contAndLogistic(TEST_MODELS, train_folders, test_folders, 1, mixed_pred_func, False, minutes)
